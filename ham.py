...
def set_2_errors(encoded):
    result = ''
    for chunk in into_blocks(encoded, length + len(bits)):
        num_bit = random.sample(list(range(0,126)),2)
        #num_bit=[31,32]
...
def decode(encoded, fix_errors=True):
    ...
    try:
        ...
        d=len(clean_chunk_list)%8
        clean_chunk_list=clean_chunk_list[:-d]
        for clean_char in [clean_chunk_list[i:i + 8] for i in range(len(clean_chunk_list)) if not i % 8]:
